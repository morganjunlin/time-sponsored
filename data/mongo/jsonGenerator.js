...
let writeStream = fs.createWriteStream('mongo.json')

function generateRecord(id) {
  let record = {};
  record.id = id;
  record.image = image[Math.floor(Math.random() * 100)];
  record.name = name1[Math.floor(Math.random() * 20)] + ' ' + name2[Math.floor(Math.random() * 10)];
  record.categories = [category[oneRandom], category[twoRandom]];
  record.ratings = rating[Math.floor(Math.random() * 6)];
  record.counts = Math.floor(Math.random() * 200) + 10;
  record.waitingtime = waitingtime[Math.floor(Math.random() * 5)]; 
  return record;
}

console.time('Finish generating JSON data')
async function dataGenerator() {
  for (let i = 1; i <= 10000000; i++) {
    if (
      !writeStream.write(
        JSON.stringify(generateRecord(i)) + (i === 10000000 ? ']' : ',')
      )
    ) {
    ...
    if (i === 10000000) {
      console.timeEnd('Finish generating JSON data')
    }
  }
}

writeStream.write('[', dataGenerator)
